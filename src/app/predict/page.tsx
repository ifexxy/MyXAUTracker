...
useEffect(() => {
  if (!shareTarget || !shareCardRef.current) return;
  ...
  const timer = setTimeout(async () => {
    try {
      ...
      canvas.toBlob(async (blob) => {
        if (!blob) { setShareTarget(null); return; }
        const fileName = `xautracker-${shareTarget.frame}-signal.png`;
        const file = new File([blob], fileName, { type: 'image/png' });
        if (typeof navigator.canShare === 'function' && navigator.canShare({ files: [file] })) {
          try { await navigator.share({ title: `${shareTarget.frame} Signal - XauTracker`, files: [file] }); }
          catch {}
        } else {
  ...
  return (
    <>
        ...
        {sigs && [
          ...
          <div key={item.key} className="tl-row">
            ...
            <button className="tl-info-btn" onClick={() => openPopup(item.key)} aria-label={`${item.frame} details`}>
             ...
             <button className="tl-share-btn" onClick={() => setShareTarget({ frame: item.frame, sig: item.sig })} aria-label={'Share ' + item.frame + ' signal'}>
      ...
      <div ref={shareCardRef} style={{
        ...
        {shareTarget && (() => {
          const s = shareTarget.sig;
          const badgeColors: Record<string, {bg: string; fg: string}> = {
            bull: { bg: '#0d2a1a', fg: '#3dba72' },
            bear: { bg: '#2a0d0d', fg: '#e05555' },
            flat: { bg: '#241b08', fg: '#c8962a' },
            wait: { bg: '#222222', fg: '#666666' },
          };
          const bc = badgeColors[s.badgeCls] || badgeColors.wait;
          return (
            <>
              {/* Header */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
                <div style={{
                  ...
                  background: 'linear-gradient(135deg, #c8962a, #f0c040)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 16, fontWeight: 900, color: '#000',
                }}>X</div>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: '-0.3px' }}>XauTracker</div>
                  <div style={{ fontSize: 11, color: '#888', marginTop: 1 }}>Gold Signal &middot; {new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
                </div>
              </div>

              {/* Divider */}
              <div style={{ height: 1, background: 'linear-gradient(90deg, rgba(200,150,42,0.4), rgba(200,150,42,0.05))', marginBottom: 24 }} />

              {/* Frame */}
              <div style={{ fontSize: 11, fontWeight: 600, color: '#888', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>
                {shareTarget.frame} Entry Signal
              </div>

              {/* Signal */}
              <div style={{ fontSize: 28, fontWeight: 800, color: '#f0f0f0', lineHeight: 1.2, marginBottom: 8 }}>
                {s.sig}
              </div>

              {/* Direction */}
              <div style={{ fontSize: 14, fontWeight: 600, color: bc.fg, marginBottom: 16 }}>
                {s.dir}
              </div>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <span style={{ fontSize: 12, color: '#888' }}>Confidence</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#f0f0f0' }}>{s.conf}%</span>
                </div>
                <div style={{ width: '100%', height: 4, background: '#222', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{
                    width: `${s.conf}%`, height: '100%',
                    background: s.conf >= 75 ? '#3dba72' : s.conf >= 55 ? '#c8962a' : '#e05555',
              ...
              {/* Reason */}
              <div style={{ fontSize: 12, color: '#888', lineHeight: 1.7, padding: '14px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)' }}>
                {s.reason}
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 20, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                <div>
                  <div style={{ fontSize: 11, color: '#666' }}>XAU/USD Spot</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: '#f0f0f0' }}>${fmtPrice(p.price)}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 11, color: '#666' }}>Daily Change</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: isUp ? '#3dba72' : '#e05555' }}>{isUp ? '+' : ''}{(p.chp || 0).toFixed(2)}%</div>
                </div>
              </div>

              {/* Footer */}
              <div style={{ fontSize: 10, color: '#555', textAlign: 'center', marginTop: 24, lineHeight: 1.5 }}>
                This signal is an algorithmic estimate &middot; Not financial advice<br />
                Generated by XauTracker &middot; {new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
              </div>
            </>
          );
        })()}
