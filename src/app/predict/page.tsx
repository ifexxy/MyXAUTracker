...
// ── Session logic (same as original) ──
function getSessionInfo() {
  ...
  let sessionMultiplier = 0.7, volLabel = 'LOW', sessionLabel = '';
  ...
  else { sessionMultiplier = 0.55; volLabel = 'MINIMAL'; sessionLabel = 'Off-Hours'; }
...
function simulateSignals(price: number) {
  ...
  const signal = (tf: string, minutes: number) => {
    ...
    const direction = drift > 0.1 ? 'bull' as const : drift < -0.1 ? 'bear' as const : 'flat' as const;
    const confidence = Math.min(90, Math.max(40, 50 + Math.abs(drift) / price * 5000));
    return {
      timeframe: tf, price: predPrice, direction,
      confidence: Math.round(confidence),
      band: `${fmtPrice(predPrice - volatility)} – ${fmtPrice(predPrice + volatility)}`,
    };
  };

  return {
    '10m': signal('10 min', 10),
    '1h': signal('1 hour', 60),
    '4h': signal('4 hour', 240),
    '24h-sig': signal('24 hour', 1440),
  };
}
...
function simulateForecasts(price: number) {
  const fc = (tf: string, minutes: number) => {
    ...
    const direction = drift > 0.3 ? 'bull' as const : drift < -0.3 ? 'bear' as const : 'flat' as const;
    const confidence = Math.min(88, Math.max(35, 45 + Math.abs(drift) / price * 4000));
    return {
      timeframe: tf, price: predPrice, direction,
      confidence: Math.round(confidence),
      band: `${fmtPrice(predPrice - volatility)} – ${fmtPrice(predPrice + volatility)}`,
      prediction: direction === 'bull' ? 'Likely to rise' : direction === 'bear' ? 'Likely to fall' : 'Range-bound',
    };
  };
  return {
    '1h': fc('1 hour', 60),
...
function calculateLevels(price: number) {
  const r2 = price + price * 0.008;
  const r1 = price + price * 0.003;
  const s1 = price - price * 0.003;
  const s2 = price - price * 0.008;
  return { r2, r1, s1, s2 };
}
...
export default function PredictPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!snap.exists()) { router.push('/signup'); return; }
  ...
  if (authLoading || accessOk === null) {
    return <div className="flex items-center justify-center p-[40px]" style={{ color: 'var(--ink-3)' }}>Loading...</div>;
  }

  if (accessOk === false) {
    return (
      <div className="flex items-center justify-center p-[24px]" style={{ minHeight: '60vh' }}>
        <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 18, padding: '32px 24px', maxWidth: 340, width: '100%', textAlign: 'center' }}>
          <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'var(--red-bg)', border: '1px solid var(--red)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: 22 }}>
            ⛔
          </div>
          ...
          <Link href="/contact" className="flex items-center justify-center gap-[8px] py-[13px] text-[14px] font-bold rounded-[10px] no-underline mb-[10px]" style={{ background: 'var(--ink)', color: 'var(--bg)' }}>
            <i className="fa-solid fa-envelope" /> Contact Us
          </Link>
          <button onClick={signOut} className="w-full py-[12px] text-[13px] font-semibold rounded-[10px] cursor-pointer flex items-center justify-center gap-[8px]" style={{ background: 'transparent', color: 'var(--ink-3)', border: '1px solid var(--border)' }}>
  ...
  const openPopup = (id: string) => {
    ...
    popupData[popId] = {
      ...
      signal: `${sig.direction === 'bull' ? '📈' : sig.direction === 'bear' ? '📉' : '⚖️'} $${fmtPrice(sig.price)}`,
    ...
    <>
          ...
          <div className="flex items-center gap-[12px] mb-[14px]">
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {userData.subscriptionStatus === 'trial' && (
                <div style={{ marginTop: 5, display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 10, fontWeight: 700, color: 'var(--gold)', background: 'var(--gold-bg)', padding: '3px 9px', borderRadius: 20 }}>
      ...
      <div className="flex items-center gap-[8px] flex-wrap px-[16px] py-[12px]" style={{ borderBottom: '1px solid var(--border)' }}>
        {[
          ...
          <div key={s.id} className="flex items-center gap-[5px] text-[10px] font-bold px-[10px] py-[5px] rounded-full uppercase"
            style={{
              color: s.active ? 'var(--green)' : 'var(--ink-3)',
              background: s.active ? 'var(--green-bg)' : 'var(--bg-2)',
              border: s.active ? 'transparent' : '1px solid var(--border)',
        ...
        <div className="flex items-center gap-[5px] text-[10px] font-bold px-[10px] py-[5px] rounded-full"
          style={{ color: 'var(--ink-3)', background: 'var(--bg-2)', border: '1px solid var(--border)', letterSpacing: '0.06em', marginLeft: 'auto' }}>
      ...
      <div style={{ padding: '14px 16px 0' }}>
        <button onClick={() => setHowToOpen(true)} className="flex items-center justify-between w-full py-[13px] px-[16px] cursor-pointer"
          style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 10 }}>
...
function createTimelineItem(sig: any) {
  const dirColor = sig.direction === 'bull' ? 'var(--green-bg)' : sig.direction === 'bear' ? 'var(--red-bg)' : 'var(--gold-bg)';
  const dirTextColor = sig.direction === 'bull' ? 'var(--green)' : sig.direction === 'bear' ? 'var(--red)' : 'var(--gold)';
  return {
    value: fmtPrice(sig.price),
    badge: sig.direction === 'bull' ? '\u{1F4C8} BULL' : sig.direction === 'bear' ? '\u{1F4C9} BEAR' : sig.direction === 'flat' ? '\u{2194}\u{FE0F} FLAT' : 'WAIT',
...
function TimelineCard({ icon, label, items, onInfo }: {
  icon: string; label: string; items: any[]; onInfo: (id: string) => void;
}) {
  return (
      ...
      {items.map((item: any, i: number) => (
        <div key={i} className="flex items-center gap-[12px] px-[16px] py-[12px]" style={{ borderBottom: i < items.length - 1 ? '1px solid var(--border)' : 'none' }}>
          <div style={{
            ...
            background: item.dot === 'live' ? 'var(--green)' : 'var(--border)',
            boxShadow: item.dot === 'live' ? '0 0 0 3px var(--green-bg)' : 'none',
          }} />
          <span style={{ fontSize: 12, color: 'var(--ink-2)', width: 52, flexShrink: 0 }}>{item.frame}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--ink)', fontVariantNumeric: 'tabular-nums', lineHeight: 1.2 }}>
              ${item.value}
            </div>
          ...
          <span style={{
            fontSize: 10, fontWeight: 700, padding: '3px 9px', borderRadius: 20, letterSpacing: '0.04em', flexShrink: 0, whiteSpace: 'nowrap',
            background: item.badgeClass === 'bull' || item.badgeClass === 'live-badge' ? 'var(--green-bg)' : item.badgeClass === 'bear' ? 'var(--red-bg)' : item.badgeClass === 'flat' ? 'var(--gold-bg)' : 'var(--bg-3)',
            color: item.badgeClass === 'bull' || item.badgeClass === 'live-badge' ? 'var(--green)' : item.badgeClass === 'bear' ? 'var(--red)' : item.badgeClass === 'flat' ? 'var(--gold)' : 'var(--ink-3)',
          }}>
            {item.badge}
          </span>
          {item.info && (
            <button onClick={() => onInfo(item.id)} className="w-[30px] h-[30px] rounded-full border flex items-center justify-center flex-shrink-0 cursor-pointer"
              style={{ border: '1px solid var(--border)', background: 'none', color: 'var(--ink-3)', fontSize: 14 }}>
...
function LevelRow({ name, value, badge, badgeClass, current }: {
  name: string; value: string; badge: string; badgeClass: string; current?: boolean;
}) {
  ...
  return (
    <div className="flex items-center justify-between gap-[10px] py-[10px]" style={{ borderBottom: '1px solid var(--border)', background: current ? 'var(--bg-2)' : 'transparent', borderRadius: current ? 8 : 0, padding: current ? '10px 8px' : '10px 0' }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%) translateY(0)', width: '100%', maxWidth: 480, background: 'var(--bg)', borderRadius: '20px 20px 0 0', borderTop: '1px solid var(--border)', zIndex: 601, padding: '0 20px 36px', maxHeight: '80vh', overflowY: 'auto' }}>
        ...
        <div style={{ background: 'var(--gold-bg)', border: '1px solid rgba(154,110,0,0.18)', borderRadius: 10, padding: '12px 14px', marginBottom: 16 }}>
          <div style={{ fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.7 }}>
            It&apos;s important to note that you should use this tool as a guide, as a personal help to making better profitable trades and not to follow the forecasts blindly, always follow the market as gold can be very intimidating even if you&apos;re an experienced trader.
          </div>
        </div>
        <button onClick={onClose} className="w-full py-[13px] mt-[16px] text-[14px] font-bold rounded-[10px] cursor-pointer flex items-center justify-center gap-[8px]"
...
function FactorRow({ icon, iconBg, iconColor, name, desc, sig, sigBg, sigColor, noBorder }: {
  ...
  name: string; desc: string; sig: string; sigBg: string; sigColor: string; noBorder?: boolean;
}) {
  return (
