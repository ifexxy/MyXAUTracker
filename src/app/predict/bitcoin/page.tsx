...
import { fetchBitcoinPrice, fmtPrice, fmtChange } from '@/lib/api';
import { getFirebase } from '@/lib/firebase';
import { doc, getDoc } from 'firebase/firestore';
import type { GoldPrice, UserData } from '@/types';
...
function estimateBtcATR(high: number, low: number, price: number, chp: number) {
  const intradayRange = high - low;
  const changeRange = Math.abs(chp / 100 * price) * 2;
  const minAtr = Math.max(price * 0.001, 50);
  const maxAtr = Math.min(price * 0.05, 5000);
  return Math.min(Math.max(Math.max(intradayRange, changeRange), minAtr), maxAtr);
}
...
function buildPredictions(price: number, chp: number, high: number, low: number): Predictions {
  const atr = estimateBtcATR(high, low, price, chp);
  ...
  function forecast(minutes: number): Forecast {
    ...
    const volPenalty = Math.min((atr / price) * 1500, 15);
...
function computeEntrySignals(
  ...
  if (poorSess) e10m = mk('WAIT', 'Low liquidity', `Off-hours session. Lower BTC volume. ATR: $${atr.toFixed(2)}`, 35, 'WAIT');
  else if (momStrong && isUp && !overbought) e10m = mk('ENTER LONG', `Bullish · +${absMom.toFixed(2)}% momentum`, `Strong momentum, not overbought (RSI≈${pseudoRSI.toFixed(0)}). 10m long scalp setup.`, btcVol ? 65 : 74, 'LONG');
  else if (momStrong && !isUp && !oversold) e10m = mk('ENTER SHORT', `Bearish · ${chp.toFixed(2)}% momentum`, `Strong downward momentum, not oversold (RSI≈${pseudoRSI.toFixed(0)}). 10m short scalp setup.`, btcVol ? 65 : 74, 'SHORT');
  ...
  let e1h: EntrySignal;
  if (poorSess) e1h = mk('WAIT', 'Low liquidity — avoid 1h entries', `Off-hours reduces follow-through. ATR: $${atr.toFixed(2)}`, 38, 'WAIT');
  else if (momStrong && isUp && !overbought) e1h = mk('ENTER LONG', `Bullish · +${absMom.toFixed(2)}% confirmed`, `Strong upward momentum, not overbought (RSI≈${pseudoRSI.toFixed(0)}). Good 1h long setup.`, btcVol ? 70 : 78, 'LONG');
  else if (momStrong && !isUp && !oversold) e1h = mk('ENTER SHORT', `Bearish · ${chp.toFixed(2)}% confirmed`, `Strong downward momentum, not oversold (RSI≈${pseudoRSI.toFixed(0)}). Good 1h short setup.`, btcVol ? 70 : 78, 'SHORT');
  ...
  if (trendBull) e24h = mk('ENTER LONG', `Strong bullish trend · +${absMom.toFixed(2)}%`, `Strong momentum in mid-range. Good daily long setup.`, btcVol ? 72 : 80, 'LONG');
  else if (trendBear) e24h = mk('ENTER SHORT', `Strong bearish trend · ${chp.toFixed(2)}%`, `Strong momentum in mid-range. Good daily short setup.`, btcVol ? 72 : 80, 'SHORT');
...
const popupData: Record<string, PopupData> = {};

export default function BitcoinPredictPage() {
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({
      autosize: true, symbol: 'BITSTAMP:BTCUSD', interval: '60',
  ...
  const atrRatio = atr / Math.max(p.price, 1);
  const atrCat = atrRatio < 0.01 ? 'LOW' : atrRatio < 0.025 ? 'NORMAL' : 'HIGH';
    ...
    return (
      <div className="flex items-center justify-center" style={{ minHeight: '100dvh', padding: 24 }}>
        <div style={{ textAlign: 'center', maxWidth: 320 }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 800, color: 'var(--ink)', marginBottom: 10, lineHeight: 1.3 }}>Sign in to access<br />Bitcoin Forecast</h2>
          <p style={{ fontSize: 13, color: 'var(--ink-3)', lineHeight: 1.65, marginBottom: 24 }}>
            Create a free account or sign in to view real-time predictions, entry signals, and market analysis for Bitcoin.
          </p>
    ...
    <>
      {user && userData && (
        <div className="predict-card" id="account-dashboard" style={{ marginTop: 14, background: 'var(--bg-2)' }}>
          ...
          <div id="dash-actions" style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {userData.subscriptionStatus !== 'active' ? (
              <Link href="/contact" className="btn-solid" style={{ textDecoration: 'none' }}>
                Contact Us
              </Link>
            ) : (
      ...
      <div className="live-strip">
        <div className="live-strip-left">
          <div>
            <div className="mini-label">BTC/USD Spot</div>
            <div className="live-price-big"><span className="sym">$</span><span id="strip-price">{fmtPrice(p.price)}</span></div>
          </div>
      ...
      <div style={{ padding: '14px 16px 0' }}>
        <button onClick={() => setHowToOpen(true)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '13px 16px', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      <div style={{ margin: '14px 16px 0', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', height: 400 }} ref={tvRef} />
      ...
      <div className="tl-outer-card">
        ...
        <div className="tl-row">
          ...
          <div className="tl-main">
            <div className="tl-value" id="tl-now-price">${fmtPrice(p.price)}</div>
            <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 3 }} id="tl-now-time">{new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</div>
          </div>
      ...
      <div className="predict-card">
        ...
        <div className="signal-grid">
          <div className="sig-gauge-row">
            <span className="sig-name" style={{ color: 'var(--green)' }}>BUY</span>
            <div className="sig-gauge-bar"><div className="sig-gauge-fill buy" style={{ width: `${marketSignal.buy}%` }} /></div>
            <span className="sig-pct" style={{ color: 'var(--green)' }}>{marketSignal.buy}%</span>
          </div>
          <div className="sig-gauge-row">
            <span className="sig-name" style={{ color: 'var(--gold)' }}>HLD</span>
            <div className="sig-gauge-bar"><div className="sig-gauge-fill hold" style={{ width: `${marketSignal.hold}%` }} /></div>
            <span className="sig-pct" style={{ color: 'var(--gold)' }}>{marketSignal.hold}%</span>
          </div>
          <div className="sig-gauge-row">
            <span className="sig-name" style={{ color: 'var(--red)' }}>SELL</span>
            <div className="sig-gauge-bar"><div className="sig-gauge-fill sell" style={{ width: `${marketSignal.sell}%` }} /></div>
            <span className="sig-pct" style={{ color: 'var(--red)' }}>{marketSignal.sell}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="predict-card">
        ...
        <div className="sentiment-arc">
          <div className="sentiment-big" id="sentiment-emoji">{sentimentEmoji}</div>
          <div className="sentiment-lbl" id="sentiment-txt">{sentimentText}</div>
        </div>
      </div>
      ...
      <div className="predict-card">
        ...
        <div className="level-row">
          <span className="level-name">Strong Resistance</span>
          <span className="level-val">${fmtPrice(r2)}</span>
          <span className="level-badge badge-resist">R2</span>
        </div>
        <div className="level-row">
          <span className="level-name">Resistance</span>
          <span className="level-val">${fmtPrice(r1)}</span>
          <span className="level-badge badge-resist">R1</span>
        </div>
        <div className="level-row current-level">
        ...
        <div className="level-row">
          <span className="level-name">Support</span>
          <span className="level-val">${fmtPrice(s1)}</span>
          <span className="level-badge badge-support">S1</span>
        </div>
        <div className="level-row">
          <span className="level-name">Strong Support</span>
          <span className="level-val">${fmtPrice(s2)}</span>
          <span className="level-badge badge-support">S2</span>
        </div>
      </div>
      ...
      <div className="predict-card">
        ...
        <div className="factor-row">
          ...
          <div className="factor-text">
            ...
            <div className="factor-desc" id="f-atr-desc">Daily range ${atr.toFixed(2)} — {atrCat.toLowerCase()} volatility environment</div>
          </div>
          <div className="factor-sig" id="f-atr-sig" style={{ background: 'var(--gold-bg)', color: 'var(--gold)' }}>${atr.toFixed(0)}</div>
        </div>
        ...
        <div className="factor-row">
          ...
          <div className="factor-text">
            ...
            <div className="factor-desc" id="f-mom-desc">{isUp ? 'Positive' : 'Negative'} {Math.abs(p.chp || 0).toFixed(2)}% move today</div>
          </div>
          <div className="factor-sig" id="f-mom-sig" style={{ background: isUp ? 'var(--green-bg)' : 'var(--red-bg)', color: isUp ? 'var(--green)' : 'var(--red)' }}>{(isUp ? '+' : '') + (p.chp || 0).toFixed(2)}%</div>
        </div>
        ...
        <div className="factor-row">
          ...
          <div className="factor-text">
            ...
            <div className="factor-desc" id="f-vol-desc">Today's range ${intradayRange.toFixed(2)} = {intradayRangePct}% of ATR</div>
          </div>
          <div className="factor-sig" id="f-vol-sig" style={{ background: 'var(--bg-3)', color: 'var(--ink-2)' }}>{volCat}</div>
        </div>
        ...
        <div className="factor-row">
          ...
          <div className="factor-sig" id="f-mr-sig" style={{ background: 'var(--gold-bg)', color: 'var(--gold)' }}>{mrLabel}</div>
        </div>
        ...
        <div className="factor-row">
          ...
          <div className="factor-text">
            ...
            <div className="factor-desc" id="f-sess-desc">{session.sessionLabel} session &middot; {session.volLabel.toLowerCase()} activity</div>
          </div>
          <div className="factor-sig" id="f-sess-sig" style={{
            background: session.sessionMultiplier >= 1.2 ? 'var(--red-bg)' : session.sessionMultiplier >= 1.0 ? 'var(--gold-bg)' : 'var(--bg-3)',
            color: session.sessionMultiplier >= 1.2 ? 'var(--red)' : session.sessionMultiplier >= 1.0 ? 'var(--gold)' : 'var(--ink-3)',
      ...
      <div className="disclaimer">
        ...
        Not financial advice. Predictions are algorithmic estimates based on ATR volatility modelling. BTC/USD carries significant market risk. Always consult a licensed financial professional before trading.
      </div>
      ...
      <footer>
        ...
        <div className="footer-disc">Trading Bitcoin carries significant financial risk. XauTracker predictions are algorithmic estimates, not financial advice.</div>
          ...
          <div className="tl-popup open" id="howto-popup">
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, background: 'var(--red-bg)', border: '1px solid rgba(184,50,50,0.18)', borderRadius: 10, padding: '12px 14px', marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.65 }}>
                <strong style={{ color: 'var(--ink)' }}>Attention:</strong> Trading Bitcoin carries high significant risk, you could lose all or part of your capital, always do your analysis, read our <Link href="/disclaimer" style={{ color: 'var(--red)', fontWeight: 700 }}>disclaimer</Link>.
              </div>
            </div>
            ...
            <div style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', marginBottom: 16 }}>
              {[
                { title: 'Entry Signals', desc: 'It forecasts how your next trade should go depending on the volality of bitcoin at that time.' },
                { title: 'Price Forecast', desc: 'It predicts what range bitcoin would be trading at at a particular time.' },
                { title: 'Market Signal', desc: 'The current market trend.' },
                { title: 'Market Sentiment', desc: 'The current market trend based on the chart.' },
                { title: 'Price Levels', desc: 'The support and resistance of the BTC/USD chart.' },
                { title: 'Model', desc: 'This carries other necessary information regarding bitcoin, what session is on, the current volatility etc.' },
              ].map((s, i, arr) => (
            ...
            <div style={{ background: 'var(--gold-bg)', border: '1px solid rgba(154,110,0,0.18)', borderRadius: 10, padding: '12px 14px', marginBottom: 16 }}>
              <div style={{ fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.7 }}>
                It&apos;s important to note that you should use this tool as a guide, as a personal help to making better profitable trades and not to follow the forecasts blindly, always follow the market as Bitcoin can be very volatile even if you&apos;re a experienced trader.
              </div>
            </div>
      ...
      {activePopup && popupData[activePopup] && (
        <>
          ...
          <div className="tl-popup open">
            <div className="tl-popup-handle" />
            <div className="tl-popup-kicker" id="tl-popup-kicker">{popupData[activePopup].kicker}</div>
            <div className="tl-popup-signal" id="tl-popup-signal">{popupData[activePopup].signal}</div>
            <div className="tl-popup-dir" id="tl-popup-dir" style={{ color: popupData[activePopup].dirColor }}>{popupData[activePopup].dir}</div>
            <div className="tl-popup-reason" id="tl-popup-reason">{popupData[activePopup].reason}</div>
            <div className="tl-popup-divider" />
            <div className="tl-popup-meta-row">
              <span className="tl-popup-meta-label">Confidence</span>
              <span className="tl-popup-meta-val" id="tl-popup-conf">{popupData[activePopup].conf}</span>
            </div>
            {popupData[activePopup].range && (
              <div className="tl-popup-meta-row" id="tl-popup-range-row">
                <span className="tl-popup-meta-label">&plusmn;1&sigma; band</span>
                <span className="tl-popup-meta-val" id="tl-popup-range">{popupData[activePopup].range}</span>
              </div>
            )}
