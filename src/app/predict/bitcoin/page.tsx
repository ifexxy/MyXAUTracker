...
/* ══════════════════════════════════════════════════════════════
   ATR ESTIMATOR (BTC-specific ranges)
══════════════════════════════════════════════════════════════ */
...
function computeEntrySignals(
  ...
  if (poorSess)
    e10m = mk('WAIT', 'Low liquidity', `Off-hours session. Lower BTC volume. ATR: $${atr.toFixed(2)}`, 35, 'WAIT');
  else if (momStrong && isUp && !overbought)
    e10m = mk('ENTER LONG', `Bullish · +${absMom.toFixed(2)}% momentum`, `Strong momentum, not overbought (RSI≈${pseudoRSI.toFixed(0)}). 10m long scalp setup.`, btcVol ? 65 : 74, 'LONG');
  else if (momStrong && !isUp && !oversold)
    e10m = mk('ENTER SHORT', `Bearish · ${chp.toFixed(2)}% momentum`, `Strong downward momentum, not oversold (RSI≈${pseudoRSI.toFixed(0)}). 10m short scalp setup.`, btcVol ? 65 : 74, 'SHORT');
  ...
  let e1h: EntrySignal;
  if (poorSess)
    e1h = mk('WAIT', 'Low liquidity — avoid 1h entries', `Off-hours reduces follow-through. ATR: $${atr.toFixed(2)}`, 38, 'WAIT');
  else if (momStrong && isUp && !overbought)
    e1h = mk('ENTER LONG', `Bullish · +${absMom.toFixed(2)}% confirmed`, `Strong upward momentum, not overbought (RSI≈${pseudoRSI.toFixed(0)}). Good 1h long setup.`, btcVol ? 70 : 78, 'LONG');
  else if (momStrong && !isUp && !oversold)
    e1h = mk('ENTER SHORT', `Bearish · ${chp.toFixed(2)}% confirmed`, `Strong downward momentum, not oversold (RSI≈${pseudoRSI.toFixed(0)}). Good 1h short setup.`, btcVol ? 70 : 78, 'SHORT');
  else if (momStrong && isUp && overbought)
  ...
  if (trendBull)
    e24h = mk('ENTER LONG', `Strong bullish trend · +${absMom.toFixed(2)}%`, 'Strong momentum in mid-range. Good daily long setup.', btcVol ? 72 : 80, 'LONG');
  else if (trendBear)
    e24h = mk('ENTER SHORT', `Strong bearish trend · ${chp.toFixed(2)}%`, 'Strong momentum in mid-range. Good daily short setup.', btcVol ? 72 : 80, 'SHORT');
...
export default function BitcoinPredictPage() {
  ...
  useEffect(() => {
    if (!mounted) return;
    try {
      const saved = localStorage.getItem('xau-btc-notif');
  ...
  useEffect(() => {
    if (!mounted) return;
    try {
      localStorage.setItem('xau-btc-notif', String(notifEnabled));
    } catch {}
  ...
  useEffect(() => {
    ...
    Object.entries(current).forEach(([frame, sig]) => {
      ...
      if (isActionable(sig) || isActionable(prev[frame])) {
        try {
          new Notification(`BTC/USD ${frame} Signal Changed`, {
            body: `${prev[frame]} → ${sig}`,
            icon: '/favicon.ico',
            tag:  `btc-signal-${frame}`,
          });
        } catch {}
      }
    });
  ...
  return (
    <>
      {/* Account Dashboard */}
      {user && userData && (
        <div className="predict-card" style={{ marginTop: 14, background: 'var(--bg-2)' }}>
      ...
      <div className="session-row">
        {[
          ...
          <div key={s.id} className={`session-chip${s.active ? ' active' : ''}`}>
            <div className="dot" />{s.label}
          </div>
        ))}
        <div className="session-chip" style={{ marginLeft: 'auto' }}>
      ...
      <div className="predict-card">
        ...
        <div className="factor-row">
          ...
          <div className="factor-sig" style={{ background: 'var(--gold-bg)', color: 'var(--gold)' }}>${atr.toFixed(0)}</div>
        </div>
          ...
          <div className="tl-popup open">
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, background: 'var(--red-bg)', border: '1px solid rgba(184,50,50,0.18)', borderRadius: 10, padding: '12px 14px', marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.65 }}>
                <strong style={{ color: 'var(--ink)' }}>Attention:</strong> Trading Bitcoin carries high significant risk. Always do your own analysis and read our{' '}
            ...
            <div style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', marginBottom: 16 }}>
              {[
                { title: 'Entry Signals',    desc: 'It forecasts how your next trade should go depending on the volatility of Bitcoin at that time.' },
                ...
                { title: 'Model',            desc: 'Carries other necessary information regarding Bitcoin, what session is on, the current volatility etc.' },
              ].map((s, i, arr) => (
            ...
            <div style={{ background: 'var(--gold-bg)', border: '1px solid rgba(154,110,0,0.18)', borderRadius: 10, padding: '12px 14px', marginBottom: 16 }}>
              <div style={{ fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.7 }}>
                Use this tool as a guide to making better trades. Do not follow forecasts blindly — always follow the market as Bitcoin can be very volatile even for experienced traders.
              </div>
            </div>
